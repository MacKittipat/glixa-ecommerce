    ...
    <table id="datatable_table" border="1">
        ...
        <?php
            foreach ($table_data as $row) {
                $order_data = $this->order_model->get_order('id', $row['order_id']);
                $user_data = $this->user_model->get_user('id', $row['user_id']);
        ?>     
            <tr>
                <td><input type="checkbox" name="chk_row[]" class="chk_row" value="<?php echo $row['id']; ?>" /></td>
                <td><?php echo order_number(get_year($order_data['order_date']), get_month($order_data['order_date']), $row['order_id']); ?></td>
                <td><?php echo $row['money']; ?></td>
                <td><?php echo $row['payment_method']; ?></td>
                <td><?php echo anchor('admin/user/profile/'.$user_data['id'], $user_data['firstname'].' '.$user_data['lastname']); ?></td>
            </tr>
        <?php
            }
        ?>
