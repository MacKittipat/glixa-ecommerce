...
class Product_review extends Controller {
    ...
    public function index() {
        if((is_user_login()==true) && (is_admin ()==true || is_super_admin()==true)) { // login และเป็น admin หรือ super_admin
            ...
            $data['page_title'] = 'รีวิวสินค้า';
            $data['page_content'] = 'admin/content/product_review_index';
            $data['page_breadcrumb'] = simple_breadcrumb(array('หน้าหลัก'=>base_url(), 'ผู้ดูแลระบบ'=>'admin', $data['page_title']=>'admin/product_review'));
            /* other code */
            $this->load->helper('table_field');
            // EDIT
            if($this->input->post('task')=='edit') {
                $this->product_review_model->edit_product_review('id', $this->input->post('edit_id'), array(
            ...
            if($this->input->post('task')=='delete') {
                foreach ($this->input->post('chk_row') as $value) { // $value = product_id
                    $this->product_review_model->edit_product_review('id', $value, array(
                        'flag_del'=>'1'
                    ));
                }
            }
            ...
            $data['tabel_field'] = array(
                                    'title'=>array(get_field_lang('shop_product_review', 'title'), '350'),
                                    'product_id'=>array(get_field_lang('shop_product_review', 'product_id'), '200'),
                                    'approve'=>array(get_field_lang('shop_product_review', 'approve'), '100'));
            // *** field ที่ใช้ search
            $data['search_field'] = array(
                                    'title'=>get_field_lang('shop_product_review', 'title'),
                                    'approve'=>get_field_lang('shop_product_review', 'approve'));
            // *** num of uri_to_assoc(num)
            $data['num_uri_to_assoc'] = 4;
            // *** page ที่ใช้ datatable
            $data['datatable_url'] = base_url().'admin/product_review/index/';
            $data['datatable_js'] = $this->load->view('admin/template/datatable_js', $data, true);
            // *** field ที่ต้องการ select
            $field = array(
                'shop_product_review.id',
                'shop_product_review.title',
                'shop_product_review.product_id',
                'shop_product_review.approve');
            // *** ชื่อ table
            $table = ' shop_product_review,shop_product ';
            // *** default WHERE
            $whe = ' WHERE shop_product_review.flag_del=0 AND shop_product_review.product_id=shop_product.id ';
            // *** default ORDER BY
            $order = ' shop_product_review.id ';
            // *** จำนวน row ต่อ page
            $limit = 10;
            ...
            // pagination
            $query_data_all = $this->db->query('SELECT shop_product_review.id FROM '.$table.' '.$whe);
            ...
            // load datatable
            $data['datatable'] = $this->load->view('admin/template/product_review_datatable', $data, true);
            // ========== DATATABLE ==========
